...
//
// This sketch shows how to use the "TouchUserInterfaceForArduino" to prompt the user
// to enter numeric and other values.  
// 
...
//
// global values set using the NumericKeyPad
//
float waterTemperature = 87.5;
int thermoCycles = 225;
...
MENU_ITEM mainMenu[] = {
  ...
  {MENU_ITEM_TYPE_COMMAND,           "Get an integer with key pad",               commandGetAnIntegerKeyPad,   NULL},
  {MENU_ITEM_TYPE_COMMAND,           "Get an float with key pad",                 commandGetAnFloatKeyPad,     NULL},
  {MENU_ITEM_TYPE_COMMAND,           "Get integer with Slider",                   commandSlider,               NULL},
...
//
// menu command show using the KeyPad to get an integer
//
void commandGetAnIntegerKeyPad(void)
{
  //
  // get the desired number of thermocycles from the user
  //
  int value = thermoCycles;
  int minThermoCycles = 1;
  int maxThermoCycles = 999999;
  boolean OKPressed = ui.numericKeyPad("Enter number of Thermocycles (1 to 9999)", value, minThermoCycles, maxThermoCycles);

  //
  // update the value if used press "OK"
  //
  if (OKPressed)
    thermoCycles = value;
}
...
//
// menu command show using the KeyPad to get a float
//
void commandGetAnFloatKeyPad(void)
{
 //
  // get the desired temperature to set water bath
  //
  float value = waterTemperature;
  float minWaterTemperature = 20;
  float maxWaterTemperature = 100;
  boolean OKPressed = ui.numericKeyPad("Enter temperature to set water (20 to 100 C)", value, minWaterTemperature, maxWaterTemperature);
...
//
// menu command that demonstrates how to get an integer value using a Slider
//
static int powerValue = 50;

void commandSlider(void)
{
  char sBuffer[25];
  ...
  // draw the title bar and clear the screen
  //
  ui.drawTitleBar("Get Integer Value Using a Slider");
  ui.clearDisplaySpace();
  ...
  myPowerSlider.centerY      = ui.displaySpaceCenterY - 10;
  ...
  // define and display "OK" and "Cancel" buttons
  //
  BUTTON okButton        = {"OK",      ui.displaySpaceCenterX-70, ui.displaySpaceBottomY-35,  120 , 35};
  ui.drawButton(okButton);

  BUTTON cancelButton    = {"Cancel",  ui.displaySpaceCenterX+70, ui.displaySpaceBottomY-35,  120 , 35};
  ui.drawButton(cancelButton);
  ...
  while(true)
  {
    ...
    //
    // process touch events on the slider
    //
    ui.checkForSliderTouched(myPowerSlider);
    ...
    if (ui.checkForButtonClicked(okButton))
    {
      ...
      // user OK pressed, get the value from the Number Box and display it
      //
      xOffsetValue = myPowerSlider.value;
      sprintf(sBuffer, "Power level = %d", xOffsetValue);
...
{  
  ...
  safetySelectionBox.centerY = 211;
